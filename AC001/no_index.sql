CREATE SCHEMA no_index001;

CREATE TABLE no_index001."crime"(id int, total_feminicide int, total_homicide int, total_felony_murder int, total_bodily_harm int, total_theft_cellphone int, total_armed_robbery_cellphone int, total_theft_auto int, total_armed_robbery_auto int, segment_id int, time_id int);

CREATE TABLE no_index001."district"(id int, name varchar, geometry varchar);

CREATE TABLE no_index001."neighborhood"(id int, name varchar, geometry varchar);

CREATE TABLE no_index001."segment"(id int, geometry varchar, oneway varchar, length varchar, final_vertice_id int, start_vertice_id int);

CREATE TABLE no_index001."time"(id int, period varchar, day int, month int, year int, weekday varchar);

CREATE TABLE no_index001."vertice"(id int, label int, district_id int, neighborhood_id int, zone_id int);
...
SELECT s.id as segment, SUM(total_feminicide) as total_feminicide, 
  ...
  FROM no_index001."crime" c, no_index001."time" t, no_index001."segment" s, 
       no_index001."vertice" v, no_index001."district" d
...
SELECT s.id as segment, 
  ...
  FROM no_index001."crime" c, no_index001."time" t, no_index001."segment" s, 
       no_index001."vertice" v, no_index001."neighborhood" n
 WHERE c.time_id = t.id
   AND t.year = 2015
   AND c.segment_id = s.id
   AND s.start_vertice_id = v.id
